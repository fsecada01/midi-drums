...
def add_intro(midi, start_bar):
    for b in range(4):
        ...
        # Crash on beat 1
        midi.addNote(0, CHANNEL, DRUM_MAP['crash'], bar_time + 0.0, 1.0, rand_velocity('ride'))
        # Kick on every quarter
        for i in range(4):
            midi.addNote(0, CHANNEL, DRUM_MAP['kick'], bar_time + i * 1.0, 1.0, rand_velocity('kick'))
        # Snare rimshot on beat 3
        midi.addNote(0, CHANNEL, DRUM_MAP['rim'], bar_time + 2.0, 1.0, rand_velocity('snare'))
...
def add_fill(midi, start_bar):
    ...
    for i in range(16):  # 16th notes
        ...
        midi.addNote(0, CHANNEL, note, bar_time + i * 0.25, 0.25, rand_velocity(kind))
...
def add_verse(midi, start_bar, bars=16):
    for b in range(bars):
        ...
        for offset in (0.0, 0.75, 2.0):
            midi.addNote(0, CHANNEL, DRUM_MAP['kick'], bar_time + offset, 1.0, rand_velocity('kick'))
        # Snare on 2 and 4
        for offset in (1.0, 3.0):
            midi.addNote(0, CHANNEL, DRUM_MAP['snare'], bar_time + offset, 1.0, rand_velocity('snare'))
        # Closed hi-hat 8ths
        for i in range(8):
            midi.addNote(0, CHANNEL, DRUM_MAP['closed_hh'], bar_time + i * 0.5, 0.5, rand_velocity('hh'))
...
def add_breakdown(midi, start_bar, bars=8):
    for b in range(bars):
        ...
        for offset in (1.0, 3.0):
            midi.addNote(0, CHANNEL, DRUM_MAP['mid_tom'], bar_time + offset, 1.0, rand_velocity('toms'))
...
def add_chorus(midi, start_bar, bars=16):
    for b in range(bars):
        ...
        for beat in range(4):
            ...
            midi.addNote(0, CHANNEL, DRUM_MAP['kick'], base, 0.5, rand_velocity('kick'))
            midi.addNote(0, CHANNEL, DRUM_MAP['kick'], base + 0.5, 0.5, rand_velocity('kick'))
        # Ride on quarters
        for i in range(4):
            midi.addNote(0, CHANNEL, DRUM_MAP['ride'], bar_time + i * 1.0, 1.0, rand_velocity('ride'))
        # Snare on 2 and 4
        for offset in (1.0, 3.0):
            midi.addNote(0, CHANNEL, DRUM_MAP['snare'], bar_time + offset, 1.0, rand_velocity('snare'))
...
def add_outro_fill(midi, start_bar):
    ...
    for i, inst in enumerate(order):
        note = DRUM_MAP['snare'] if inst == 'snare' else DRUM_MAP[inst]
        kind = 'snare' if inst == 'snare' else 'toms'
        midi.addNote(0, CHANNEL, note, bar_time + i * 1.0, 1.0, rand_velocity(kind))
    # Crash at bar end
    midi.addNote(0, CHANNEL, DRUM_MAP['crash'], bar_time + 3.75, 1.0, rand_velocity('ride'))
