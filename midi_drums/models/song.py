...
from .pattern import Pattern, TimeSignature
...
class PatternVariation:
    ...
    pattern: Pattern
    probability: float = 0.3  # Chance this variation will be used instead of base
    bars: Optional[List[int]] = None  # Specific bars to apply variation, None = any
...
class Section:
    ...
    def should_add_fill(self, bar_number: int, fill_frequency: float) -> Optional[Fill]:
...
class Song:
    ...
    global_parameters: Optional[GenerationParameters] = None
    metadata: Dict[str, Any] = field(default_factory=dict)

    def __post_init__(self):
        """Validate song parameters."""
        if not 60 <= self.tempo <= 300:
            raise ValueError(f"Tempo must be between 60-300 BPM, got {self.tempo}")
    ...
    @classmethod
    def create_simple_structure(cls, name: str, tempo: int = 120,
                              genre: str = "rock", style: str = "default") -> 'Song':
        """Create a song with basic verse-chorus structure."""
        from .pattern import Pattern  # Import here to avoid circular imports
