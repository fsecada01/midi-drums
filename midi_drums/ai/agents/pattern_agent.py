...
SYSTEM_PROMPT = (
    ...
    "IMPORTANT — song completeness: the user's description tells you the "
    "genre/style/CHARACTER of the song (e.g. what the opening riff feels like), "
    "it does NOT limit how many sections the song has. Treat any part of the "
    "description that only mentions an intro or a riff as flavor for that one "
    "section, not as the whole song. Unless the user explicitly asks for a "
    "short excerpt, a single section, or 'just the intro/riff', a --song request "
    "means a complete, radio-length arrangement with real dynamic contrast — "
    "at minimum 6-8 sections covering intro, verse, chorus (or hook), a second "
    "verse/chorus pass, a bridge or breakdown, and an outro. A song with only "
    "an intro and one verse is an incomplete failure, not a valid minimal output.\n\n"
    "When composing a song, follow this exact workflow:\n"
    "1. Infer genre, style, tempo, and a FULL structure (6-8+ sections, see above) "
    ...
    "6. Before summarising, verify: does the song have at least 6 sections, and does "
    "it include a chorus/hook and an outro (not just intro+verse)? If not, you are "
    "not done — go back and add/generate/assign the missing sections now.\n"
    "7. After all assignments, briefly summarise what was created and reference the IDs\n\n"
